...
$logFile = $logDir . '/reservation_edit.log';
error_log("예약 수정 시작: " . date('Y-m-d H:i:s') . "\n", 3, $logFile);
...
try {
    ...
    $sql = "UPDATE reservations SET 
        ...
        accommodation_start = :accommodation_start,
        accommodation_end = :accommodation_end,
        accommodation_type = :accommodation_type,
        accommodation_room_count = :accommodation_room_count,
    ...
    // 쿼리 실행
    $stmt->execute($params);
    error_log("예약 수정 성공 (ID: {$_POST['id']})\n", 3, $logFile);

    // 성공 메시지와 함께 리다이렉트
    header("Location: reservation_list.php?status=success&message=" . urlencode("예약이 수정되었습니다."));
    exit;

} catch (Exception $e) {
    error_log("에러 발생: " . $e->getMessage() . "\n", 3, $logFile);
    header("Location: reservation_list.php?status=error&message=" . urlencode($e->getMessage()));
    exit;
}
?> 